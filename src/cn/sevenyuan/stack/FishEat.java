...
public class FishEat {
    ...
    public static int solution(int[] fishSize, int[] fishDirection) {
        ...
        for (int i = 0; i < fishNumber; i++) {
            ...
            // 如果栈中还有鱼，并且栈中鱼向右，当前的鱼向左游动，那么就有相遇的可能性
            while (!stack.empty() && fishDirection[stackTopIndex] == right && curFishDirection == left) {
                // 如果栈顶的鱼比较大，那么将新来的鱼吃掉
                if (fishSize[stackTopIndex] > curFishSize) {
                    hasEat = true;
                    break;
                }
                // 如果栈中的鱼较小，那么把栈中的鱼吃掉，需要弹出栈
                stack.pop();
            }
            // 如果新来的鱼没有被吃掉，将它的位置压入栈中
            if (!hasEat) {
                stack.push(i);
            }
        }
